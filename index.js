...
function spawnFood() {
    var food;
    do {
        food = field.children[Math.floor(Math.random() * FIELD_SIZE)]
                    .children[Math.floor(Math.random() * FIELD_SIZE)];
...
function snakeMotion() {
    ...
    if (nextCell && (nextCell.className != "snake-cell")) {
        ...
        var newGame = confirm("You lose. Create a new game?");
        if (newGame) {
            document.getElementsByTagName("div")[0].removeChild(field);
            startGame();
        }
